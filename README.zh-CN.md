...
PEM 是 OpenSSL、Java/BouncyCastle、证书和通用密码库中很常见的格式，因此
GMKit 会继续保留 PEM 输出。三未信安等加密机如果提示需要“密码设备应用接口”
定义的密钥格式，通常需要 SDF/ECCref 二进制结构，而不是 PEM。
...
导出 SDF/ECCref 明文密钥，用于符合 GM/T 0018/SDF 接口格式的加密机导入：
...
- `sm2.sdf.pri`：`ECCrefPrivateKey`，长度 68 字节
- `sm2.sdf.pub`：`ECCrefPublicKey`，长度 132 字节

默认结构：

```text
ECCrefPrivateKey = uint32 bits + 64字节K
ECCrefPublicKey  = uint32 bits + 64字节x + 64字节y
bits             = 256，默认小端：00 01 00 00
K/x/y            = 32字节前导00填充 + 32字节SM2值
```

如果设备文档明确要求大端 `bits` 字段，可以加：
